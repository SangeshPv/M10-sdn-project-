...
/* If the protocol is not TCP, return NF_ACCEPT to allow the packet to continue */
...
/* SYN + FIN packets are considered stealth scans and are not valid in normal TCP communication.
 * If both SYN and FIN flags are set, increment the syn_fin_counter and log a warning.
 */
